...
Kinds is a class in `com.sun.tools.javac.code` with a user disclaimer. It contains a more precise categorization of all the diiferent available kinds then the more general ElementKind. 
...
But on the other hand, in the method bodies of Symbol the `<Symbol>.kind` is ubiquitous. The Kinds class plays a big role, and the precise definition of what Kind of symbols exist seems to have been extremely useful to the makers.
