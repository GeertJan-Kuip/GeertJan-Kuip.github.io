...
It has been a while since I wrote my last blog post and the reason for my absence is me being immersed in my project. Just writing and structuring code, it's sproceeding fine.

While writing, I found myself applying patterns, many of which I learned from the java.compiler and jdk.compiler library, from the Clean Architecture book (see a previous post about it) or from I don't know where. In this post I'll describe some of these patterns, including my motivation for using them.
...
Speaking about using Node as abstract representation for different implementation types, one of the classes where this comes in handy is the class that is responsible for doing graph calculation on a set of Node objects and a set of Edge objects. Library JGraphT can use any type of object as node (it's own terminology speaks of 'vertex').  

I made the class that applies methods of JGraphT on my set of nodes and edges generic, in a rather specific way. This is the list of fields at the start of my ` JGraphComputations` class:
...
I already wrote about this topic earlier, and I think I have a sort of deep understanding of how and why this is such a good concept. 

In my appllication I decided that my package `symbolanalysis`, that is responsible for the compilation of the source files and the generation of sets of `Relation` objects that describe relationships between classes as pairs of Symbol objects, should be independent. But as this `symbolanalysis`, that lives at the core of the application, requires files to compile, it is naturally dependent on the package that is respnible for the capturing of the source files. 

What I did was to create an interface 'JavaFilesSupplier' in the independent `symbolanalysis` package:
...
This interface is implemented by class `JavaFilesContainer` that lives in the `filemanagement` package. This is the first part of its code:
...
One thing not to forget: if you want to make a package or module independent and use dependency inversion, you must make sure that every type you use in the interface (either method arguments or return values) is part of the independent package/module as well. In this case, `JavaFile` must live in the independent module. If it would live in `filemanagement`, both packages `filemanagement` and `symbolanalysis` would be each other's dependencies. It is explained clearly in Clean Architecture, that arrows between modules must all point in the same direction.
