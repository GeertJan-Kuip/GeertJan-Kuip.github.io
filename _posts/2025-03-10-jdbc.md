...
- If you pass query string to execute(), executeUpdate() or executeQuery(), an SQLException is thrown during runtime. Reason is that Statement, superinterface of preparedStatement, has method signatures for these methods that allow for the passing of a string.
- If you pass the query string to the .execute() method, you get a runtime SQLException, not a compile error. 
...
- The bind variables in querystrings net to be set like this:
...
- There should be "throws SQLException" in the method signature. 
...
* There are 6 set.. types:
    ...
    * setObject
    * setString
- You can use setObject for any value, those values will be autoboxed. You can cast them back but why should you.
...
- Use execute() if no ResultSet is to be returned. executeUpdate() is not used in CallableStatement.
...
- For a stored procedure with no IN- and only OUT, use execute(). This example from book shows how to retrieve the OUT value:
