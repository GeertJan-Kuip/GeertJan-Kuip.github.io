...
| Method | Description |
|---------|-------------|
|ExecutorService NewSingleThreadExecutor() |single thread, unbounded queue, sequential processing in order of submission |
|ScheduledExecutorService newSingleThreadScheduledExecutor()|single thread, schedules commands after given delay or to execute periodically |
|ExecutorService newCachedThreadPool()|creates thread pool, new threads are created as needed and idle old ones are reused |
|ExecutorService newFixedThreadPool(int)|creates fixed number of threads operating of a shared unbounded queue |
|ScheduledExecutorService newScheduledThreadPool(int)|creates thread pool that can schedule commands to run after given delay or to execute periodically |
