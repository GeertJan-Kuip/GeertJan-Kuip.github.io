...
The app that I'm working on must have a good design especially with regards to the dependencies between the different packages. The more business logic there is in a package, the less dependencies on other packages it should have, preferably zero.
...
- Identify the concrete class in B that A needs to import. Let's say this class is named 'Implementation'.
...
The downside of these typical domain objects is that you have to create a new one every time you solve a new problem. They are so tailor-made that they can often not be reused, they increase the amount of boiler plate and they force the people who want to understand your code to learn the specifics of your domain objects over and over again.

Compare this to the more simple and basic objects like `List<T>` or `Map<P,R>`. These structures do not require extra code, especially when you do not fill them with custom objects, and they are so universal that you do not even have to be a Java developer to understand them. In functional programming lists are the central element and you can do almost everything with them.

With regards to the decoupling of components, it is beneficial to use basic data types form the standard library instead of custom domain objects. The dependencies that come with basic types are rock solid (java.util) and they are part of the vocabulaire of any Java developer. So generally: using basic types to pass around values between components can be a good idea.

## My application

I'm talking about it already for months now and I am definitely progressing, especially with regards to my understanding of the Java compiler internals which I need to work with. 
...
This package utilizes java.compiler and jdk.compiler to sort out all the data required to make a complete UML diagram. It recognizes packages and imports, it dissects classes and finds both the dependencies between classes and the dependencies of the package as a whole on outer packages. It stores the information in two types of custom domain objects, namely 'nodes' and 'edges'. Furthermore it has to store the non-resolved dependencies somehow.

The domain objects will be stored as interface reference types, whereby the interface file is part of package 2.

### Package 3 - creating representations

Package 3 depends on two, which is sort of unavoidable given the complex data that needs to be passed. The dependency is softened because interface reference types are being used. 

Based on the data in the Node, Edge and eventually other objects, package 3 is able to create all sorts of representations of the data. It can output different sorts of textfiles that can be used for Mermaid diagrams, uml or other, and it can provide all sorts of representations of the source code (html, only fields/signatures, colorized text, html).

It has access to the Node and Edge objects via its interfaces, meaning there is a clear contract between package 2 and package 3, whereby the contract is managed by package 2. 