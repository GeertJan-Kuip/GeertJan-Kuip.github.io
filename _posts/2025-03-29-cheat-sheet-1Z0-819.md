...
- Each case statement requires the keyword case, followed by a vulue and a colon.
...
In general, the value must be resolved at compile time, so a value that is the result of a method running at runtime is not allowed. Furthermore, it must match the tye in the switch statement (of course).
